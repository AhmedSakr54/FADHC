...
class DN_modified(nn.Module):
    def __init__(self, in_channels=3):
        super(DN_modified, self).__init__()
        self.DRDB_layer1 = DRDB(in_ch=in_channels, growth_rate=32)
        self.conv1 = nn.Conv2d(in_channels, 24, 3, 2, 1) # Scale 1/2
        self.DRDB_layer2 = DRDB(in_ch=24, growth_rate=32)
        self.conv2 = nn.Conv2d(24, 48, 3, 2, 1)          # Scale 1/4 (Target for FADHC 2)
        self.DRDB_layer3 = DRDB(in_ch=48, growth_rate=32)
        self.conv3 = nn.Conv2d(48, 96, 3, 2, 1)          # Scale 1/8 (Target for FADHC 1)
        self.DRDB_layer4 = DRDB(in_ch=96, growth_rate=32)
        self.conv4 = nn.Conv2d(96, 128, 3, 2, 1)         # Scale 1/16
    ...
    def forward(self, x):
        ...
        x1 = self.conv2(x1)
        feat_s4 = x1  # Capture 48 channels
        
        # Scale 1/4 -> 1/8
        x1 = self.DRDB_layer3(x1)
        x1 = self.conv3(x1)
        feat_s8 = x1  # Capture 96 channels

        # Scale 1/8 -> 1/16
        x1 = self.DRDB_layer4(x1)
        x1 = self.conv4(x1) # Deepest bottleneck
        
        # Decoder path
        x1 = self.DRDB_layer5(x1)
        x1 = self.up1(x1)
        # FIX: Force alignment to feat_s8 size (Handle odd input sizes)
        if x1.shape[-2:] != feat_s8.shape[-2:]:
            x1 = F.interpolate(x1, size=feat_s8.shape[-2:], mode='bilinear', align_corners=False)

        x1 = self.DRDB_layer6(x1)
        x1 = self.up2(x1)
        # FIX: Force alignment to feat_s4 size
        if x1.shape[-2:] != feat_s4.shape[-2:]:
            x1 = F.interpolate(x1, size=feat_s4.shape[-2:], mode='bilinear', align_corners=False)
        ...
        # Final safety alignment to input size
        if x1.shape[-2:] != x.shape[-2:]:
             x1 = F.interpolate(x1, size=x.shape[-2:], mode='bilinear', align_corners=False)

        final_depth = self.final_conv(x1)

        return final_depth, feat_s8, feat_s4
    
class DN(nn.Module):
    def __init__(self, in_channels=3):
        super(DN, self).__init__()
        self.DRDB_layer1 = DRDB(in_ch=in_channels, growth_rate=32)
        self.conv1 = nn.Conv2d(in_channels, 24, 3, 2, 1)
        self.DRDB_layer2 = DRDB(in_ch=24, growth_rate=32)
